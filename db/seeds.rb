...
beauty = Category.create(name: "Beauty General")

# ---- Amazon All-Beauty Seeds ---- #
file_path = File.join(__dir__, "amazon_datasets/all_beauty.json")
serialised_All_beauty = File.read(file_path)
parsed_all_beauty = JSON.parse(serialised_All_beauty)

parsed_all_beauty.each do |product| puts 
  name = product["title"]
  p name
  ...
  Product.new(name: name, description: description, price: price.to_s, asin: asin, image_url: image_url, brand: brand )
...
# Create products for product details (use existing data) - 3pm
